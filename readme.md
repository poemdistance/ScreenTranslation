...
* #### 有一定几率程序崩溃, 具体原因未知，只知道是由Xlib引起，详见src/bugRecord.log, 崩溃后管控程序会自动重新运行，但是存在进程没杀干净的问题，目前需要手动清理
* ####  有一定几率会发生鼠标监控程序异常现象，导致取词动作全部被丢弃，不弹出图标，已知浅层原因是InNewWin自锁变量在newNormalWin退出清零后仍然读出1，此时不要用任务栏图标退出程序，请手动清理所有进程，此异常情况发生后作者发现使用SIGTERM信号时，鼠标动作监控程序好像收不到，使用SIGKILL才最终杀掉所有残留进程。不好复现，还没找到内在原因。
...
* #### 相关设置说明
  * 快捷键除了播放音频和切换翻译源显示外，其他都不允许只有一个键作为快捷键，必须带上Modifier，比如Control，Shift等，另外如果Gnome使用了Super键作为Overview功能，带Super键的快捷键设置会无效。
