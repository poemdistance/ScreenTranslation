### 作者正在爬取离线库，完成后准备添加发布离线功能，速度不敢太快，可能再有几天就爬取完成了，总词数约38万。
...
1. 程序连续选中同一个文本，翻译图标在第二次不会弹出，是为了解决一些错误弹出而采用的逻辑 
 
2. 运行后长时间未使用软件，在第一次双击后可能会不弹出图标，或者翻译结果呈现上一次未知的复制，是因为复制操作有延时，程序获取成了上一次的复制结果
 
3. 程序有较小几率呈现出上一次的翻译结果，原因是上一次结果获取较慢，直到在这一次中才呈现出来，打乱了步伐

4. 程序有较小几率显示不完美，也跟翻译速度影响有关

5. 百度或者谷歌翻译如果在翻译过程中出了问题，可能是网络原因，也可能是代码本身的Bug，由于翻译结果的不确定性，不保证已经将全部因素考虑进去，有可能翻译某些特定情况的时候使程序奔溃，重启便可。如果多次选中同一个文本都产生崩溃，那便是代码的问题。如果重启后一切恢复，便是网络问题，即有可能是网络不好，或者百度的反爬虫机制导致的。

6. 如果想协助作者完善此软件，请在程序崩溃后将日志文件 `/var/log/mstran.log`发送至作者github同名gmail邮箱:`poemdistance@gmail.com` 
 
7. 软件是有桌面图标的，安装成功并重启后按Super键，搜索Mstran即可找到，点击图标会打开或者重启翻译软件。

8. 以上部分问题是由于设计产生的，有些实属无奈，虽非下策，却也属中策，后期如果发现可行替代方案，将会一一完善。
