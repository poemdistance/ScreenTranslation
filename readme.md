...
### 2020-3-11 更新说明：添加任务栏图标，右键图标后选择`Settings`可进入设置窗口，设置包含：
* #### 相关快捷键自定义设置
* #### 弹出图标消失时间设置
...
### 2020-3-11 新更新目前已知问题(复现困难，不好解决):
* #### 有一定几率程序崩溃, 具体原因未知，只知道是由Xlib引起，详见src/bugRecord.log
* #### shortcutListener.c 有一定几率导致CPU飙升,发生在用任务栏图标重启程序的时候，select不断返回1导致没有休眠的不断死循环，具体内部原因未知，杀掉所有翻译进程线程后重启可解决。
* #### 程序有一定几率在双击选中文本后不弹出图标，原因未知。（使用 quick search 正常弹出窗口后恢复正常，没有复现，不敢保证）

##### 新更新变动：百度在线翻译已改为必应翻译，原因：百度翻译不仅针对爬虫，连浏览器打开的翻译界面超过一定时间后连接同样失效，体验很差，经常超时，暂时弃用之

##### 再更新: 添加暂时忽略功能，快捷键 Alt+V 后选中特定软件的窗口，翻译软件关闭之前都不会在该窗口弹出翻译图标，如需取消，再次Alt+V选中同一个软件窗口即可。
...
### 4. 截图翻译    --  屏幕上短时间内点击不同的3个点触发(或Alt+d)，形成的三角形不能过小 <br><br>
  * 为尽可能防止误触截图功能，三个点基本在一条线上的情况会被抛弃，如果触发了之后想取消本次截图识别，点击鼠标右键即可 
